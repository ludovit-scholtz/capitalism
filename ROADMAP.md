...
- [ ] (85%) Power Plant System & Energy Economics is live in core simulation flows: every building now tracks configurable power priority, city load shedding keeps high-priority buildings online first, the tick engine applies `POWERED`/`CONSTRAINED`/`OFFLINE` operational states, and GraphQL now exposes `energyGridStatus`, `cityEnergyOverview`, `buildingEnergyStatus`, and `setPowerPriority` for UI/automation consumers.
