...
- [ ] Enforce OIDC HTTPS metadata outside Development: set `"RequireHttpsMetadata": true` in the base `appsettings.json` of both `projects/Api` and `projects/MasterApi`, override to `false` only in `appsettings.Development.json`, default `MasterApi/Security/BiatecOidcOptions.RequireHttpsMetadata` to `true`, and add a fail-fast non-Development startup guard with a regression test.
- [ ] Add a scheduled (cron) trigger to `api-ci-cd.yml`, the master API deploy workflow, and both frontend CI/CD workflows so Docker images are rebuilt and redeployed monthly after .NET patch Tuesday, ensuring runtime security patches (e.g. CVE-2026-45591, fixed in 10.0.9) ship even when `main` has no code pushes.
- [ ] Fix nginx `add_header` inheritance in `projects/frontend/nginx.conf` and `projects/master-frontend/nginx.conf`: move the security headers into a shared snippet included in every location that declares its own `add_header` (static assets, `/sw.js`, `/health`) so asset responses keep `nosniff` and HSTS; add the missing `Permissions-Policy` header to the game frontend and remove the stale gtag inline-script hash from its CSP.
- [ ] Clear esbuild advisory GHSA-g7r4-m6w7-qqqr (dev-server arbitrary file read on Windows, low, dev-tooling only) in both frontends at the next dependency cycle: run `npm audit fix` or bump Vite to a release that lifts esbuild past 0.28.0, then re-run `npm audit` to confirm zero advisories.
