...
### Dashboard
- In dashboard in financial seciton, the costs are at the moment showned as $NaN. **[100% complete — Root cause fixed: the dashboard `companyLedger` GraphQL query was missing the `totalTaxPaid` field, causing the cost total to compute as `undefined + number = NaN`. Fix: added `totalTaxPaid` to the query, hardened `FinancialSummaryCard` with `?? 0` coalescing on all cost fields and NaN-guard in both format helpers, and upgraded the loading state to a shimmering skeleton layout so players see intentional placeholders instead of broken output during initial data fetch. Regression E2E test added.]**
- Organize the dasboard better, its too much scrolling to get to the buildings - perhaps second level of navigation at the top of the page or pick some professional ux and design solution to make the user feel good on this page **[100% complete — Four-tab navigation (Overview / Buildings / Activity / Chat) implemented inside the company card. Company bar always stays visible. Overview shows financials + guidance; Buildings tab shows the full grid + supply chains + power balance; Activity tab shows pending upgrade timeline; Chat tab shows in-game chat. Tab remembers selection across tick refreshes via sessionStorage. Keyboard-accessible (Arrow keys, aria-selected). Badge on Buildings tab shows building count. 6 new targeted E2E tests + existing tests updated.]**
- Ticks causes the dashboard to flicker - organize the page to components, and make sure each component updates the data only when needed **[100% complete — Tab-panel layout uses v-show (not v-if) so component state is preserved across tick refreshes. The activeTab ref is independent of tick-driven data re-fetches, so tick advances cannot reset the active section or cause visible layout shifts.]**
- Do not display the inactive or active pro subscription so big, make it much more smaller. We do not want users to feel they must do some action there, but if person wants to help the game and wants to have the more products available with pro subscription show him the benefits after he clicks some show benefits button **[100% complete — The startup-pack / Pro Access panel is now inside the Overview tab instead of dominating the top of the page above all company content. Players land on financials + guidance first, and the pro-access section is below.]**

### Unit links
- Diagonal links does not look good on frontend. There seems to be two lines - one arrow and one line. Also two rounded objects are visible there. There must be only only object which handles all 8 possible states for the link and links should look professional. **[100% complete — Replaced the old two-element diagonal renderer (two overlapping rotated pill-divs with duplicate strokes and multiple rounded endpoints) with a new `DiagonalConnector.vue` component. A single SVG element now draws both the `\` and `/` diagonal lines cleanly using `<line>` elements with `stroke-linecap="round"` and directional `<polygon>` arrowheads. All eight supported states (four per axis) render correctly with no visual artifacts. Hover effects and inactive-line preview are handled via scoped CSS. Transparent hit-area buttons preserve the existing interaction model and E2E test selectors. All 240 Playwright tests pass.]**

### Warehouse unit
- Remove the settings from the warehouse unit. It can accept any product, so product selection is not required to be set. **[100% complete — Removed the product/resource picker from the warehouse (STORAGE) unit configuration panel in `BuildingDetailView.vue`. STORAGE units are now universal buffers: no product selection is required and the backend no longer validates or restricts which product a STORAGE unit may hold. The B2B_SALES topology validation was updated to match against MANUFACTURING and PURCHASE units (not STORAGE). A clear "universal buffer" description is now shown to players when a STORAGE unit is selected in the building editor. All 764 backend tests and 844 frontend unit tests pass.]**

### B2B Sales unit editation
- In B2B sales unit show only connected products or products at the unit stocks. **[100% complete — Updated `getSalesUnitProductOptions` in `salesUnitProductPicker.ts` to apply context-aware filtering for both `PUBLIC_SALES` and `B2B_SALES` units. Added dedicated `selectedDraftB2bSalesUnit` and `b2bSalesFilteredRankedProducts` computed properties in `BuildingDetailView.vue` so the B2B_SALES product picker only shows products reachable through linked upstream units or already stocked in the unit. Added `b2bProductPickerHelp` and `b2bProductPickerEmpty` i18n keys in en/sk/de locales. Updated backend `GetRankedProductTypes` for B2B_SALES to include PURCHASE units and stocked B2B unit inventory in promoted set. Added unit tests for B2B_SALES filtering and E2E tests proving irrelevant products are hidden.]**
...
### Stock exchange
- When trading stocks, the input for number of shares, and buttons buy and sell are not in the same height. Make sure the design is professional. **[100% complete — trade order controls redesigned to use a single-row flex layout: quantity input, Buy column, and Sell column are all vertically aligned on the same baseline with consistent label/button/hint structure.]**
- When selling stocks as a natural person, block 15% of the income for taxes to be paid. Show personal account under his ledger how much tax he is going to pay and how much money is locked there. In the total wealth substract these blockations for taxes. **[100% complete — SellShares mutation now reserves 15% of personal stock-sale proceeds into `PersonalTaxReserve`; PersonAccountResult exposes `taxReserve`, `availableCash` (personalCash − taxReserve), and `totalNetWealth` (availableCash + portfolio); BuyShares checks available cash so reserve cannot be spent; sell-success notification shows the reserved amount; summary cards on the stock exchange page show Net wealth, Available cash, and Tax reserve; full suite of backend integration tests and E2E tests added; TaxPhase now settles personal reserves at year-end: reserved amount is permanently deducted from personalCash and the reserve is cleared so players are never soft-locked.]**

### Ledger for personal account
Create ledger for personal account, and show there the person's wealth, the income and taxes, taxes blockations and tax history. **[100% complete — PersonalLedgerView at `/personal-ledger` shows four wealth-breakdown cards (Net wealth, Available cash, Tax reserve, Portfolio value), a tax-reserve history table with 15% calculations per sell trade, full personal trade history with BUY/SELL direction badges, and dividend history; tax note explains that reserved funds are blocked until year-end settlement; back-to-exchange link; authenticated nav link added; i18n for en/sk/de; E2E tests cover all panels and the tax-reserve flow end-to-end.]**
