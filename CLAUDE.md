...
Mobile client mirroring `projects/frontend`'s screens/nav against the same game GraphQL API — see `projects/flutter_app/README.md` and `.github/copilot-instructions.md` → `## Flutter mobile app` for full detail. For running/writing tests specifically, use the shared skill at `.github/skills/flutter-app-testing/SKILL.md` (readable by both Claude Code and GitHub Copilot — keep it as the single source of truth for test-running conventions rather than duplicating guidance here). Key points:
- **Every screen in `ROADMAP.md` → `### Flutter mobile app` is now implemented** — design system, auth, onboarding, dashboard, and every economy/social/building/banking/market/admin screen through Tutorial and all 6 Operations admin screens. Several carry deliberate trims from full web parity (Building Detail, Ledger, Bank Management, and all 6 Operations screens have the largest ones) — each is documented in its own file's top-of-file comment and in `ROADMAP.md`; check there before assuming byte-for-byte parity on a specific screen. Remaining: the read-only live-server verification pass against the real stage server.
...
- **Never show a raw tick number to a player** — convert it to in-game calendar time first. `lib/core/utils/game_time.dart`'s `formatGameTickTime(tick, languageCode)` mirrors the web's `gameTime.ts` (fixed epoch `2000-01-01T00:00:00Z`, 1 tick = 1 in-game hour — unrelated to `tickIntervalSeconds`, the real-world seconds between ticks). The raw tick is still surfaced for debugging via the `GameTickTime` widget (`lib/core/widgets/game_tick_time.dart`), which shows the formatted time and puts the tick number in a long-press/hover `Tooltip`; use that widget (or call `formatGameTickTime` directly inside a `Tooltip` for interpolated strings) at every new tick-display site rather than interpolating `tick` into a `Text` directly.
- **All money/number display must go through `lib/core/utils/app_number_format.dart`'s `AppNumberFormat`** (mirrors the web's `currencyFormat.ts`: `compactMoney`/`money`/`moneyByFieldSize`/`currencyTitle`/`number`/`compactNumber`), never a hand-rolled `'\$${amount.toStringAsFixed(0)}'` or bare `NumberFormat.simpleCurrency(...)` — those don't honor the player's chosen app language and don't abbreviate large values consistently. Every formatter takes an explicit `languageCode` (from `context.watch<LocaleState>().languageCode`, `lib/core/i18n/locale_state.dart`), not the device locale.
- **App language is a `LocaleState`/`SharedPreferences`-backed player preference** (`lib/core/i18n/`, key `app_locale`, supported codes `en`/`sk`/`de` matching the web's `SUPPORTED_LOCALES` — `kSupportedAppLanguages`/`languageDisplayNames` in `app_locale.dart`), switched on the mobile-only Settings screen (`lib/features/settings/settings_screen.dart`, Drawer → App → Settings, alongside About) and threaded into `MaterialApp.locale` in `app.dart` so `GlobalMaterialLocalizations` picks it up too — defaults explicitly to English rather than following the device locale (same "no silent device-preference fallback" reasoning as the dark-first theme rule above). Any widget test that pumps a screen using `AppNumberFormat`/`GameTickTime`/`formatGameTickTime` needs a `ChangeNotifierProvider<LocaleState>` in its tree (the shared `pumpCapitalismApp()` harness already provides one) or it throws `ProviderNotFoundException`.
- **`DateFormat` (used by `game_time.dart`) throws `LocaleDataException` for any non-`'en'` locale until its symbol data is loaded** — unlike `NumberFormat`, which works for every locale with zero setup. `main.dart` calls `initializeDateFormatting()` once at startup before `runApp()`; `flutter test` never runs `main()`, so `test/support/app_harness.dart`'s `pumpCapitalismApp()` calls the same thing (guarded to run once), and any pure (non-widget) `test()` that calls `game_time.dart` functions directly needs its own `setUpAll(() => initializeDateFormatting())`.
