...
- **`lib/features/exchange/`** ports the 4 exchange screens as separate files. `global_exchange_screen.dart` ports `GlobalExchangeView.vue`'s Resources tab (real `globalExchangeOffers`/`buyFromExchange`, buy dialog resolving STORAGE/PURCHASE units + company bank accounts) and a read-only Products tab. `stock_exchange_screen.dart` ports `StockExchangeView.vue`'s listings table only — buying/selling moved to its own screen (`stock_trading_screen.dart`, `/stock/trade/:companyId`, ports `StockTradingView.vue`: order book, limit orders, shareholders) rather than the web's inline per-row trade panel; dividend proposals/voting, company mergers, CEO replacement, and hostile takeover are **not** ported. `forex_exchange_screen.dart` ports `ForexExchangeView.vue`'s Swap/Rates/History tabs only — the Transfer tab (generic, not forex-specific) and the entire Gold AMM tab are skipped.
