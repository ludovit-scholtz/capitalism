...
- **Testing uses `flutter_test`/`WidgetTester`** (built into the SDK) as the default — no emulator/device or native build required for most coverage. `createAppRouter()` in `lib/core/router/app_router.dart` is a factory, not a bare singleton `GoRouter`, specifically so tests can build a fresh router per test instead of leaking navigation state across `pumpWidget` calls, and it also accepts an injectable `UrlOpener`/`http.Client` for the same reason (`HomeScreen` makes a real GraphQL call — thread a fake `http.Client`, e.g. `MockClient` from `package:http/testing.dart`, through `createAppRouter(httpClient: ...)`, or use the shared `pumpCapitalismApp()` harness in `test/support/app_harness.dart` which does this by default). `test/support/InMemoryTokenStorage` stands in for `SecureTokenStorage` so tests don't hit the real secure-storage platform channel. **The drawer's `ListView` only mounts elements within the viewport + cache extent** (true even for a non-lazy `ListView(children: ...)`), so navigation tests that assert on lower drawer sections/items must call `tester.binding.setSurfaceSize(const Size(800, 2400))` (see `test/navigation_test.dart`) before pumping, or the assertions will spuriously fail with "0 widgets found" for sections that are technically present but off the default 800×600 test viewport. **Platform-specific branches can be unit-tested without owning all three toolchains** by temporarily setting `debugDefaultTargetPlatformOverride` (`package:flutter/foundation.dart`) around the call under test and restoring it in a `finally` — see `test/biatec_oidc_service_test.dart`'s redirect-URI-shape test for the pattern. Add the official `integration_test` package once more screens exist and true device-level end-to-end journeys matter (the mobile equivalent of Playwright for `projects/frontend`) — `flutter_test` alone can't exercise platform channels or real network calls.
