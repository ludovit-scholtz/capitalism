...
- **Most screens are still empty placeholders**, built from the shared `lib/core/widgets/placeholder_screen.dart` widget, which renders bare content only — no nested `Scaffold`/`AppBar` — because it's always shown as the `body` of `AppShell`'s outer `Scaffold` via the go_router `ShellRoute`; wrapping it in a second `Scaffold` would stack a duplicate app bar under the real one. Home, all four auth screens (Sign In, Forgot Password, Reset Password, Auth Callback), Onboarding, Dashboard, News, Notifications, Contracts, Leaderboard, Player Profile, Cities, World Map, Encyclopedia, Resource Detail, Building Market, Buy Building, Sell Building, Building Detail (trimmed), all 6 City tabs, Global Exchange, Stocks, Stock Trading, Forex, Ledger, Company Contracts, Company Settings, Company Research, Personal Ledger, Loan Marketplace, Bank Management, Bank Loan Request, Bank Statement, Market Intelligence, Market Dashboard, Energy Market, Global Events, Marketing Analytics, Trade Routes, and the Discord/Chat nav actions are implemented; see below. Each remaining screen file/class names the Vue view it mirrors (e.g. `HomeScreen` → `HomeView.vue`). See `ROADMAP.md` → `### Flutter mobile app` for the per-screen implementation backlog; implement screens by porting the matching Vue view's behavior, not by inventing new UX.
