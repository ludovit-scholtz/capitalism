...
- Root repository currently contains the frontend app at `projects/frontend`.
...
- Always run `npm run lint` after i18n or locale-test changes. The locale unit tests themselves are linted in CI, and `no-explicit-any` errors in `src/i18n/__tests__/i18n.test.ts` can fail `frontend-ci-cd` even when unit tests, Playwright, and builds are all green.
