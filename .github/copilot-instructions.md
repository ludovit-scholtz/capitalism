...
- **`lib/features/banking/banking_screens.dart`** ports the four banking screens, sharing `banking_models.dart`/`banking_service.dart`. Loan Marketplace (`/banking`) has Borrow/Deposit tabs; Bank Management (`/bank/:buildingId`) **folds the web's separate owner and customer sub-views into one screen** driven by an `bankInfo.lenderCompanyId` ownership check, and skips the dynamic deposit-rate-change-with-history UI (wired in the service, not exposed here); Request Loan (`/bank/:buildingId/request-loan`) is one form against the real `acceptLoan` mutation, simplified from the web's 4-step wizard; Bank Statement (`/bank-statement[/:companyId]`) is the real paginated ledger with the same unauthenticated-redirect guard as Dashboard/Forex.
