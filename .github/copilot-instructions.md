...
- Mobile client providing the same game-playing interface as `projects/frontend`, talking to the same game GraphQL API. Native platform folders (`android/`, `ios/`, `web/`, `windows/`) are generated and committed; `flutter analyze` is clean and `flutter build web` succeeds. Building for Android/iOS/Windows additionally needs their native toolchains (Android SDK; Visual Studio "Desktop development with C++" workload) installed on the dev machine.
- **Every screen is currently an empty placeholder** built from the shared `lib/core/widgets/placeholder_screen.dart` widget, which renders bare content only — no nested `Scaffold`/`AppBar` — because it's always shown as the `body` of `AppShell`'s outer `Scaffold` via the go_router `ShellRoute`; wrapping it in a second `Scaffold` would stack a duplicate app bar under the real one. Each screen file/class names the Vue view it mirrors (e.g. `HomeScreen` → `HomeView.vue`). See `ROADMAP.md` → `### Flutter mobile app` for the per-screen implementation backlog; implement screens by porting the matching Vue view's behavior, not by inventing new UX.
...
- **Testing uses `flutter_test`/`WidgetTester`** (built into the SDK) for widget and navigation tests today — no emulator/device or native build required, which is why it's the right default while most screens are placeholders. `createAppRouter()` in `lib/core/router/app_router.dart` is a factory, not a bare singleton `GoRouter`, specifically so tests can build a fresh router per test instead of leaking navigation state across `pumpWidget` calls; `test/support/InMemoryTokenStorage` stands in for `SecureTokenStorage` so tests don't hit the real secure-storage platform channel. **The drawer's `ListView` only mounts elements within the viewport + cache extent** (true even for a non-lazy `ListView(children: ...)`), so navigation tests that assert on lower drawer sections/items must call `tester.binding.setSurfaceSize(const Size(800, 2400))` (see `test/navigation_test.dart`) before pumping, or the assertions will spuriously fail with "0 widgets found" for sections that are technically present but off the default 800×600 test viewport. Add the official `integration_test` package once real screens exist and true device-level end-to-end journeys matter (the mobile equivalent of Playwright for `projects/frontend`) — `flutter_test` alone can't exercise platform channels or real network calls.
