...
public sealed class JwtSessionRevocationIntegrationTests(ApiWebApplicationFactory factory) : IClassFixture<ApiWebApplicationFactory>
{
    ...
    {
        var token = await RegisterAndGetTokenAsync("revocation-a@capitalism.test");
    ...
    {
        var targetToken = await RegisterAndGetTokenAsync("revocation-target@capitalism.test");
        var unaffectedToken = await RegisterAndGetTokenAsync("revocation-unaffected@capitalism.test");
        ...
        var targetBody = await targetMe.Content.ReadAsStringAsync();
        Assert.Contains("session_revoked", targetBody, StringComparison.OrdinalIgnoreCase);
    ...
    {
        var oldToken = await RegisterAndGetTokenAsync("revocation-refresh@capitalism.test");
        var currentToken = await LoginAndGetTokenAsync("revocation-refresh@capitalism.test");
    ...
    {
        var token = await RegisterAndGetTokenAsync("revocation-cleanup@capitalism.test");
        ...
        var revokedToken = await db.RevokedTokens.FindAsync(jti);
        ...
        var afterCleanup = await db.RevokedTokens.FindAsync(jti);
    ...
    private async Task<string> RegisterAndGetTokenAsync(string email)
    {
        var result = await ExecuteGraphQlAsync(
            ...
            """,
            new { input = new { email, displayName = "Revocation Test", password = "TestPass123!" } });
    ...
    private static string CreateAdminToken()
    {
        var claims = new[]
        {
            ...
            new Claim(ClaimTypes.Email, "admin@capitalism.local"),
