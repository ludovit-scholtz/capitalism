  ...
  <div class="container py-8 px-4 max-w-4xl">
    ...
    <div class="mb-6">
      <RouterLink to="/dashboard" class="inline-flex items-center gap-1.5 text-sm text-muted hover:text-brand no-underline transition-colors">
        <span>ÔćÉ</span> {{ t('buildingDetail.backToDashboard') }}
      </RouterLink>
    </div>
        ...
        <div v-if="selectedType" class="mb-8 flex flex-col gap-4">
          ...
          <div v-if="selectedType === 'MEDIA_HOUSE'" class="flex flex-col gap-1.5">
            ...
            >
              ...
              <option value="NEWSPAPER">­čô░ {{ t('cityMap.mediaTypeNewspaper') }} (├Ś1.0)</option>
              <option value="RADIO">­čô╗ {{ t('cityMap.mediaTypeRadio') }} (├Ś1.5)</option>
              <option value="TV">­čô║ {{ t('cityMap.mediaTypeTv') }} (├Ś2.0)</option>
            </select>
            ...
            <div class="mt-2 grid gap-2">
              <div class="rounded-lg border border-divider bg-surface p-3">
                <p class="mb-1.5 text-xs font-bold text-foreground">­čô░ {{ t('cityMap.mediaTypeNewspaper') }}</p>
                <p class="text-xs text-muted">{{ t('cityMap.mediaTypeGuideNewspaper') }}</p>
              </div>
              <div class="rounded-lg border border-divider bg-surface p-3">
                <p class="mb-1.5 text-xs font-bold text-foreground">­čô╗ {{ t('cityMap.mediaTypeRadio') }}</p>
                <p class="text-xs text-muted">{{ t('cityMap.mediaTypeGuideRadio') }}</p>
              </div>
              <div class="rounded-lg border border-divider bg-surface p-3">
                <p class="mb-1.5 text-xs font-bold text-foreground">­čô║ {{ t('cityMap.mediaTypeTv') }}</p>
              ...
              <div class="rounded-lg border border-amber-300/50 bg-amber-500/10 p-3">
                <p class="mb-1 text-xs font-bold text-amber-700 dark:text-amber-400">­čĺí {{ t('cityMap.mediaTypeStrategyTitle') }}</p>
          ...
          <div
            ...
            aria-live="polite"
          >
            <span class="text-xl flex-shrink-0" aria-hidden="true">ÔÜá´ŞĆ</span>
          ...
          <div v-if="selectedType === 'BANK'" class="flex gap-4 mt-1 p-5 bg-[rgba(59,130,246,0.08)] border border-[rgba(59,130,246,0.25)] rounded-lg">
            <span class="text-3xl flex-shrink-0">­čĆŽ</span>
            <div class="flex-1">
          ...
          >
            <span class="text-xl flex-shrink-0" aria-hidden="true">{{ companyHasBankCapital ? 'Ôťů' : 'ÔÜá´ŞĆ' }}</span>
        ...
        <div v-if="selectedType && selectedCityId" class="mb-8">
          ...
          <div v-if="selectedLot" class="flex flex-col gap-2 mt-4 p-4 border border-divider rounded-lg bg-page">
            ...
            <div v-if="selectedType === 'MINE' && selectedLot.resourceType" class="buy-building-mining-summary" data-testid="buy-building-mining-summary">
              <h4 class="buy-building-mining-summary-title">ÔŤĆ {{ t('cityMap.miningDepositSummaryTitle') }}</h4>
