...
async function loadDashboardData() {
  const companiesData = await gqlRequest<{ myCompanies: Company[] }>(
    `{ myCompanies {
      ...
      buildings { id name type level cityId powerStatus lotMaterialQuantity lotOriginalMaterialQuantity destroyedAtUtc hasDefaultedCollateralLoan units { id unitType gridX gridY level } }
    } }`,
  )
