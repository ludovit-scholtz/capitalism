...
// Feature flag: set VITE_AUTH_PASSWORD_ENABLED=true to show the password form.
// Defaults to false so OIDC-only deployments skip the form entirely.
...
// initiate the OIDC flow after a short delay so the user sees a brief loading state.
onMounted(() => {
  if (!passwordAuthEnabled && !requiresConsentRetry.value) {
    setTimeout(() => {
      handleBiatecSignIn()
    }, 500)
  }
  ...
  <main class="container pb-16 pt-6 lg:pb-20 lg:pt-8">
    <section class="mx-auto flex min-h-[calc(100vh-64px-5rem)] max-w-md items-center justify-center py-4 lg:py-6">
      <div class="flex w-full flex-col gap-6 rounded-2xl border border-divider bg-card p-6 shadow-lg sm:p-8 lg:p-10">
        ...
        <!-- Auto-redirect state when password auth is disabled -->
        ...
        <!-- Normal form (password auth enabled, or consent retry) -->
        <template v-else>
