...
import type { CampaignAnalyticsResult, CampaignAnalyticsRow } from '@/types'

const { t } = useI18n()

const props = defineProps<{
  analytics: CampaignAnalyticsResult
  filteredRows: CampaignAnalyticsRow[]
}>()

const roiLabel = computed(() => {
  const spend = props.analytics.totalMarketingSpend
  const rev = props.analytics.totalRevenue
...
<template>
  ...
  <div v-else class="ca-rows">
    <article v-for="row in filteredRows" :key="row.buildingUnitId" class="ca-row-card" :aria-label="rowTitle(row)">
      <!-- Card header -->
...
/* Empty state within analytics block */
.ca-empty-state {
  text-align: center;
  padding: 3rem 1rem;
  color: var(--color-text-muted);
