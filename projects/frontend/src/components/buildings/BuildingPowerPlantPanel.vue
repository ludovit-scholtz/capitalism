...
<div v-if="building?.type === 'POWER_PLANT'" class="power-plant-analytics-panel mb-4 rounded-xl border border-divider bg-card p-4 sm:p-5" role="region" aria-label="power plant analytics">
  ...
  <template v-else-if="powerPlantAnalytics">
    ...
    <div v-if="powerPlantAnalytics.timeline.some(s => s.surplusIncome > 0 || s.gridFine > 0 || s.operatingCosts > 0)" class="ppa-chart mt-4 flex h-14 items-end gap-px overflow-hidden rounded-md border border-divider bg-surface px-1 py-1" role="img" :aria-label="t('powerPlant.analytics.panelTitle')">
      <div
