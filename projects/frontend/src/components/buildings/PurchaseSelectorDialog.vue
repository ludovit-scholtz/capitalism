...
<div v-if="showPurchaseSelector" class="purchase-selector-page fixed inset-0 z-50 overflow-y-auto bg-black/45 p-4 backdrop-blur-sm sm:p-6" role="dialog" :aria-label="t('buildingDetail.purchaseSelector.title')">
  <div class="purchase-selector-shell mx-auto mt-2 w-full max-w-6xl rounded-2xl border border-divider bg-card p-4 shadow-2xl sm:mt-6 sm:p-6">
    ...
    <div class="purchase-selector-grid grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-5">
      ...
      <section class="purchase-selector-card rounded-xl border border-divider bg-surface p-4">
        ...
        <button type="button" class="purchase-vendor-card mt-3 grid w-full gap-0.5 rounded-xl border border-divider bg-card p-3 text-left transition-colors hover:border-primary/40 hover:bg-primary/5" :class="{ selected: selectedDraftPurchaseUnit?.vendorLockCompanyId == null, 'border-primary bg-primary/10': selectedDraftPurchaseUnit?.vendorLockCompanyId == null }" @click="selectPurchaseVendor(null)">
        ...
        <button
          ...
          :class="{ selected: selectedDraftPurchaseUnit?.vendorLockCompanyId === building?.companyId, 'border-primary bg-primary/10': selectedDraftPurchaseUnit?.vendorLockCompanyId === building?.companyId }"
        ...
        <div v-if="purchaseVendorOptions.length > 0" class="purchase-vendor-list mt-3 grid gap-2">
          <button
            ...
            :class="{ selected: selectedDraftPurchaseUnit?.vendorLockCompanyId === option.companyId, 'border-primary bg-primary/10': selectedDraftPurchaseUnit?.vendorLockCompanyId === option.companyId }"
