...
<template>
<div v-if="building?.type === 'APARTMENT' || building?.type === 'COMMERCIAL'" class="property-panel mb-4 rounded-xl border border-divider bg-card p-4 sm:p-5" role="region" aria-label="property management">
  ...
  <div v-if="building?.pendingPricePerSqm != null" class="pending-rent-notice mt-4 flex items-start gap-2 rounded-lg border border-amber-300/60 bg-amber-500/10 p-3 text-sm text-amber-800 dark:text-amber-300" role="status">
  ...
  <div v-if="building?.occupancyPercent === 0 && building?.pricePerSqm == null" class="property-empty-state mt-4 rounded-lg border border-dashed border-divider bg-surface-muted px-3 py-2 text-sm text-muted">
