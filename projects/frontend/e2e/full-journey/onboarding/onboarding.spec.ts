 ...
 */
async function getGuestProfitRevenue(page: Page, industry: string, productName: string, _companyName = `${industry} Revenue Corp`): Promise<number> {
  setupMockApi(page)
  await page.goto('/onboarding')
  await chooseOnboardingRouteChoices(page, { industry, product: productName })
...
/** Drives the guest wizard through the lot-purchase steps (no auth) and lands on the save-progress screen. */
async function completeGuestSteps1to4(page: Page, _companyName = 'Guest Corp') {
...
async function completeGuidedOnboarding(page: Page, _companyName: string) {
  await chooseOnboardingRouteChoices(page, { industry: 'Furniture', product: 'Wooden Chair' })
 ...
 */
async function completeGuidedOnboardingForIndustry(page: Page, _companyName: string, industryLabel: string, productLabel: string) {
  await chooseOnboardingRouteChoices(page, { industry: industryLabel, product: productLabel })
...
test.describe('Dashboard account switcher', () => {
  test('switches between person and company views from the top menu', async ({ page }) => {
    ...
    await page.locator('.ctx-account-option', { hasText: player.displayName }).click()
    ...
    await page.locator('.ctx-trigger').click()
    await page.locator('.ctx-account-option', { hasText: 'Alpha Manufacturing' }).click()
  ...
  test('personal account can create a new company from the dashboard', async ({ page }) => {
    ...
    await expect(page.locator('.ctx-trigger')).toContainText('Delta Ventures')
  })
})
...
test.describe('Onboarding wizard', () => {
  ...
  test('industry selection advances directly to product step', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
  ...
  test('shows three starter products for the selected industry', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    await expect(page.locator('.product-card', { hasText: 'Wooden Chair' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Wooden Table' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Wooden Bed' })).toBeVisible()
  })

  test('product step shows the Food Processing starter set (AC3/AC5)', async ({ page }) => {
    ...
    await expect(page.locator('.product-card', { hasText: 'Pasta' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Crackers' })).toBeVisible()
  ...
  test('product step shows the Healthcare starter set (AC3/AC5)', async ({ page }) => {
    // AC3: Each starter industry presents a viable starter product/path.
    // AC5: The onboarding flow prepares an initial factory layout appropriate to the selected industry.
    ...
    await expect(page.locator('.product-card', { hasText: 'Bandages' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'First Aid Kit' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Wooden Chair' })).toHaveCount(0)
  ...
  test('industry cards show first product hint for each starter industry', async ({ page }) => {
    ...
    // Only free-tier industries are visible to unauthenticated guests.
    // Pro-only cards (Electronics, Construction, Pharmaceuticals, Energy, Logistics) are hidden until login.
    setupMockApi(page)
    await page.goto('/onboarding')
    await chooseOnboardingCity(page)
  ...
  test('industry card descriptions explain the business fantasy', async ({ page }) => {
    ...
    // Pharmaceuticals description explains gold-based pharma synthesis
    await expect(page.locator('.industry-card', { hasText: 'Pharmaceuticals' }).locator('.card-desc')).toContainText('gold')
    // Energy description explains coal-to-fuel conversion
    await expect(page.locator('.industry-card', { hasText: 'Energy' }).locator('.card-desc')).toContainText('coal')
    // Logistics description explains cotton packaging chain
    await expect(page.locator('.industry-card', { hasText: 'Logistics' }).locator('.card-desc')).toContainText(/cotton/i)
  })
...
test.describe('Guest onboarding wizard', () => {
  ...
  test('guest progress is stored in the route query instead of localStorage', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/industry=FURNITURE/)
    await expect(page).toHaveURL(/productId=prod-chair/)
    await expect(page).toHaveURL(/cityId=city-ba/)
    await expect(page).toHaveURL(/factoryLotId=lot-industrial-2/)
  ...
  test('guest progress survives page refresh and can still be migrated', async ({ page }) => {
    ...
    await chooseOnboardingRouteChoices(page, { industry: 'Furniture', product: 'Wooden Chair' })
    await expect(page).toHaveURL(/step=factory/)
    await expect(page).toHaveURL(/industry=FURNITURE/)
    await expect(page).toHaveURL(/productId=prod-chair/)
    await expect(page).toHaveURL(/cityId=city-ba/)
    await expect(page).toHaveURL(/ipoRaiseTarget=200000/)
  ...
  test('analytics events fire for industry_selected, city_selected, and factory_configured during guest steps (AC 12)', async ({ page }) => {
    ...
    // Verifies: onboarding_start → industry_selected → product_selected → city_selected → ipo_selected
    // → factory_configured → shop_configured.
    setupMockApi(page)
    ...
    expect(names).toContain('product_selected')
    expect(names).toContain('city_selected')
    expect(names).toContain('ipo_selected')
...
test.describe('Full onboarding journey', () => {
  test('register → onboard → dashboard shows new company', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Dashboard' })).toBeVisible()
    await expect(page.locator('.company-card')).toHaveCount(1)
    await expect(page.locator('.company-card').first().locator('h2').first()).toBeVisible()
  })
...
test.describe('Onboarding resume and progress persistence', () => {
  test('resumes at correct step after page refresh mid-onboarding', async ({ page }) => {
    ...
    // Should resume on the product step with selected industry preserved in route state
    await expect(page.getByRole('heading', { name: 'Choose Your First Product' })).toBeVisible()
  })
...
test.describe('Onboarding skip and re-entry behavior', () => {
  test('player who navigates away mid-flow can re-enter and resume from correct step', async ({ page }) => {
    ...
    // Re-enter onboarding via saved URL state — should resume on product step
...
test.describe('Onboarding on narrow/mobile layouts', () => {
  ...
  test('industry cards are visible and selectable on a narrow (375px) viewport', async ({ page }) => {
    ...
    // Selecting a card should auto-advance to product step on mobile
    await page.locator('.industry-card', { hasText: 'Furniture' }).click()
    await expect(page.getByRole('heading', { name: 'Choose Your First Product' })).toBeVisible()
  })
...
test.describe('Onboarding budget coaching — guest cash visibility (AC 6)', () => {
  ...
  test('guest wizard step 3 shows starting cash and post-purchase cash budget panel', async ({ page }) => {
    ...
    // Move to factory-lot step
    await chooseOnboardingRouteChoices(page, { industry: 'Furniture', product: 'Wooden Chair' })

    // Factory lot step — budget panel must be visible before any lot is selected
    await expect(page.getByRole('heading', { name: 'Choose Your First Factory Lot' })).toBeVisible()
  ...
  test('guest wizard shop-lot step shows available cash and selected starter product', async ({ page }) => {
    setupMockApi(page)
    await page.goto('/onboarding')

    // Move to shop-lot step
    await chooseOnboardingRouteChoices(page, { industry: 'Furniture', product: 'Wooden Chair' })
    await page.getByRole('button', { name: 'List View' }).click()
    await chooseStarterFactoryLot(page)
    await page.getByRole('button', { name: 'Purchase First Factory' }).click()

    // Shop lot step — available cash + selected product coaching must be visible
    await expect(page.getByRole('heading', { name: 'Choose Your First Shop Lot' })).toBeVisible()

    // Available cash must be shown (coaching the player on their remaining budget)
    await expect(page.getByText('Available cash')).toBeVisible()

    // Selected starter product context must remain visible in budget coaching cards
    const selectedProductCard = page.locator('.budget-card', { hasText: 'Selected starter product' })
    await expect(selectedProductCard).toContainText('Wooden Chair')
  })
...
test.describe('Factory and shop layout display after completion', () => {
  ...
  test('authenticated completion screen shows factory layout with units from backend', async ({ page }) => {
    ...
    await authenticateViaLocalStorage(page, `token-${player.id}`)
    await page.goto('/onboarding')
    await completeGuidedOnboarding(page, 'Auth Test Corp')
...
test.describe('Onboarding wizard progress bar accuracy (AC1 — visible step progress)', () => {
  ...
  test('progress bar is hidden on the completion step (step 7)', async ({ page }) => {
    ...
    // Completion/save-progress screen hides the progress bar
...
test.describe('Empty-lots graceful degradation (AC11)', () => {
  ...
  test('step 4 shows no-shop-lots message when city has no suitable sales-shop lots', async ({ page }) => {
    ...
    // Move to factory-lot step (factory purchase succeeds — factory lot IS available)
    await chooseOnboardingRouteChoices(page, { industry: 'Furniture', product: 'Wooden Chair' })
    await page.getByRole('button', { name: 'List View' }).click()
    await chooseStarterFactoryLot(page)
    await page.getByRole('button', { name: 'Purchase First Factory' }).click()
...
test.describe('City selection — Vienna as starter city', () => {
  test('guest can complete wizard steps 1-5 after selecting Vienna as starter city', async ({ page }) => {
    ...
    // Select Vienna (not Bratislava or Prague)
    await chooseOnboardingRouteChoices(page, { industry: 'Furniture', product: 'Wooden Chair', city: 'Vienna' })
...
test.describe('Guest migration — all starter industries (AC2, AC9, AC13)', () => {
  ...
  test('guest can register and migrate Furniture progress in Prague city to authenticated account', async ({ page }) => {
    ...
    await chooseOnboardingRouteChoices(page, {
      industry: 'Furniture',
...
test.describe('Guest save-progress conversion step — city, industry, and keeps list (AC summary)', () => {
  ...
  test('Save Your Progress section shows what-you-keep list with company, city, and product', async ({ page }) => {
    ...
    // Company identity must appear in the list
    await expect(saveSectionList).toContainText(/Company/i)
...
test.describe('IPO plan — Expansion option ($600k raise, 25% founder)', () => {
  test('Expansion IPO card shows $600k raise and 25% founder ownership', async ({ page }) => {
    // Expansion option uses a 25% founder stake with the maximum 3x raise from the $200k founder contribution.
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    ...
    await expect(expansionIpoCard).toContainText('€555,555.56')
  ...
  test('Expansion IPO updates Cash after purchase when factory lot is selected', async ({ page }) => {
    ...
    // card must reflect $800,000 - lot price = correct remaining balance.
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    ...
    // Bratislava (EUR): €740,741 - €90,000 = €650,741
...
test.describe('Guest onboarding — Expansion IPO selection', () => {
  test('guest can select Expansion IPO and wizard shows $800k starting company cash', async ({ page }) => {
...
test.describe('Onboarding wizard — localization (AC10)', () => {
  test('step 1 heading and Next button appear in Slovak when app locale is sk', async ({ page }) => {
    ...
    await page.locator('.industry-card', { hasText: 'Furniture' }).click()
    await expect(page.getByRole('heading', { name: /produkt/i })).toBeVisible()
  })
...
test.describe('Electronics industry — Pro-gated starter path', () => {
  test('Electronics industry card appears in the industry selection step', async ({ page }) => {
    ...
    // Electronics card must appear alongside the three free industries and 4 other Pro industries
    await expect(page.locator('.industry-card', { hasText: 'Electronics' })).toBeVisible()
    await expect(page.locator('.industry-card')).toHaveCount(8)
  })
  ...
  test('non-Pro authenticated user cannot select Electronics and sees an error', async ({ page }) => {
    ...
    const electronicsCard = page.locator('.industry-card', { hasText: 'Electronics' })
    await expect(electronicsCard.locator('.industry-pro-badge')).toBeVisible()
    // The card is aria-disabled for non-Pro users; dispatch click to assert guard handler behavior
    await electronicsCard.dispatchEvent('click')
...
test.describe('Construction Pro-gated onboarding', () => {
  test('Construction industry card shows PRO badge for all users', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    await expect(constructionCard.locator('.industry-pro-badge')).toContainText('PRO')
  })

  test('non-Pro authenticated user cannot select Construction and sees an error', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const constructionCard = page.locator('.industry-card', { hasText: 'Construction' })
    await expect(constructionCard.locator('.industry-pro-badge')).toBeVisible()
    await constructionCard.dispatchEvent('click')
  ...
  test('Pro subscriber can select Construction and advance to product step', async ({ page }) => {
    const proExpiry = new Date(Date.now() + 7_200_000).toISOString()
    const player = makePlayer({ proSubscriptionEndsAtUtc: proExpiry })
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    ...
    await page.locator('.industry-card', { hasText: 'Construction' }).click()

    await expect(page.getByRole('heading', { name: 'Choose Your First Product' })).toBeVisible()
  })

  test('Construction product step shows the three starter products', async ({ page }) => {
    const proExpiry = new Date(Date.now() + 7_200_000).toISOString()
    const player = makePlayer({ proSubscriptionEndsAtUtc: proExpiry })
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    ...
    await expect(page.locator('.product-card', { hasText: 'Residential Block' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Commercial Block' })).toBeVisible()
  ...
  test('Construction industry card has correct content (first-product hint and why-tag)', async ({ page }) => {
    ...
    await expect(constructionCard.locator('.industry-pro-badge')).toBeVisible()
    await expect(constructionCard.locator('.card-first-product')).toBeVisible()
    await expect(constructionCard.locator('.card-why')).toBeVisible()
  })
})

test.describe('Pharmaceuticals Pro-gated onboarding', () => {
  test('Pharmaceuticals industry card appears in the industry selection step', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    await expect(page.locator('.industry-card', { hasText: 'Pharmaceuticals' })).toBeVisible()
  })

  test('Pharmaceuticals card shows a PRO badge', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const pharmCard = page.locator('.industry-card', { hasText: 'Pharmaceuticals' })
    await expect(pharmCard.locator('.industry-pro-badge')).toBeVisible()
    await expect(pharmCard.locator('.industry-pro-badge')).toContainText('PRO')
  })

  test('non-Pro user cannot select Pharmaceuticals and sees an error', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const pharmCard = page.locator('.industry-card', { hasText: 'Pharmaceuticals' })
    await expect(pharmCard.locator('.industry-pro-badge')).toBeVisible()
    await pharmCard.dispatchEvent('click')
  ...
  test('Pro subscriber can select Pharmaceuticals and advance to product step', async ({ page }) => {
    const proExpiry = new Date(Date.now() + 7_200_000).toISOString()
    const player = makePlayer({ proSubscriptionEndsAtUtc: proExpiry })
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    ...
    await page.locator('.industry-card', { hasText: 'Pharmaceuticals' }).click()

    await expect(page.getByRole('heading', { name: 'Choose Your First Product' })).toBeVisible()
  })

  test('Pharmaceuticals product step shows the three starter products', async ({ page }) => {
    const proExpiry = new Date(Date.now() + 7_200_000).toISOString()
    const player = makePlayer({ proSubscriptionEndsAtUtc: proExpiry })
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    ...
    await expect(page.locator('.product-card', { hasText: 'Aspirin' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Vitamin Capsule' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Antibiotic' })).toBeVisible()
  })

  test('Pharmaceuticals card shows first product hint and why-choose tagline', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const pharmCard = page.locator('.industry-card', { hasText: 'Pharmaceuticals' })
    await expect(pharmCard.locator('.card-first-product')).toContainText('Aspirin')
    await expect(pharmCard.locator('.card-why')).toContainText('Inelastic demand')
  })
})

test.describe('Energy Pro-gated onboarding', () => {
  test('Energy industry card appears in the industry selection step', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    await expect(page.locator('.industry-card', { hasText: 'Energy' })).toBeVisible()
  })

  test('Energy card shows a PRO badge', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const energyCard = page.locator('.industry-card', { hasText: 'Energy' })
    await expect(energyCard.locator('.industry-pro-badge')).toBeVisible()
    await expect(energyCard.locator('.industry-pro-badge')).toContainText('PRO')
  })

  test('non-Pro user cannot select Energy and sees an error', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const energyCard = page.locator('.industry-card', { hasText: 'Energy' })
    await expect(energyCard.locator('.industry-pro-badge')).toBeVisible()
    await energyCard.dispatchEvent('click')

    await expect(page.getByRole('alert')).toBeVisible()
    await expect(page.getByRole('alert')).toContainText('Pro subscription')
  })

  test('Pro subscriber can select Energy and advance to product step', async ({ page }) => {
    const proExpiry = new Date(Date.now() + 7_200_000).toISOString()
    const player = makePlayer({ proSubscriptionEndsAtUtc: proExpiry })
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    ...
    await page.goto('/onboarding')
    await chooseOnboardingCity(page)
    await page.locator('.industry-card', { hasText: 'Energy' }).click()

    await expect(page.getByRole('heading', { name: 'Choose Your First Product' })).toBeVisible()
  })

  test('Energy product step shows the three starter products', async ({ page }) => {
    const proExpiry = new Date(Date.now() + 7_200_000).toISOString()
    const player = makePlayer({ proSubscriptionEndsAtUtc: proExpiry })
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    ...
    await expect(page.locator('.product-card', { hasText: 'Coal Briquette' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Heating Oil' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Industrial Fuel' })).toBeVisible()
  })

  test('Energy card shows first product hint and why-choose tagline', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const energyCard = page.locator('.industry-card', { hasText: 'Energy' })
    await expect(energyCard.locator('.card-first-product')).toContainText('Coal Briquette')
    await expect(energyCard.locator('.card-why')).toContainText('Steady demand')
  })
})

test.describe('Logistics Pro-gated onboarding', () => {
  test('Logistics industry card appears in the industry selection step', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    await expect(page.locator('.industry-card', { hasText: 'Logistics' })).toBeVisible()
  })

  test('Logistics card shows a PRO badge', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const logisticsCard = page.locator('.industry-card', { hasText: 'Logistics' })
    await expect(logisticsCard.locator('.industry-pro-badge')).toBeVisible()
    await expect(logisticsCard.locator('.industry-pro-badge')).toContainText('PRO')
  })

  test('non-Pro user cannot select Logistics and sees an error', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const logisticsCard = page.locator('.industry-card', { hasText: 'Logistics' })
    await expect(logisticsCard.locator('.industry-pro-badge')).toBeVisible()
    await logisticsCard.dispatchEvent('click')

    await expect(page.getByRole('alert')).toBeVisible()
    await expect(page.getByRole('alert')).toContainText('Pro subscription')
  })

  test('Pro subscriber can select Logistics and advance to product step', async ({ page }) => {
    const proExpiry = new Date(Date.now() + 7_200_000).toISOString()
    const player = makePlayer({ proSubscriptionEndsAtUtc: proExpiry })
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    ...
    await page.goto('/onboarding')
    await chooseOnboardingCity(page)
    await page.locator('.industry-card', { hasText: 'Logistics' }).click()

    await expect(page.getByRole('heading', { name: 'Choose Your First Product' })).toBeVisible()
  })

  test('Logistics product step shows the three starter products', async ({ page }) => {
    const proExpiry = new Date(Date.now() + 7_200_000).toISOString()
    const player = makePlayer({ proSubscriptionEndsAtUtc: proExpiry })
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    ...
    await expect(page.locator('.product-card', { hasText: 'Shipping Bag' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Storage Sack' })).toBeVisible()
    await expect(page.locator('.product-card', { hasText: 'Cargo Pack' })).toBeVisible()
  })

  test('Logistics card shows first product hint and why-choose tagline', async ({ page }) => {
    const player = makePlayer()
    const state = setupMockApi(page, { players: [player] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    ...
    const logisticsCard = page.locator('.industry-card', { hasText: 'Logistics' })
    await expect(logisticsCard.locator('.card-first-product')).toContainText('Shipping Bag')
