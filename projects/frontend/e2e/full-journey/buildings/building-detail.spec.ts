...
test.describe('Building detail upgrades', () => {
  ...
  test('diagonal link reversal: tr-bl (↙) replaced by bl-tr (↗) — AC4 other axis', async ({ page }) => {
    // Proves AC4 for the second diagonal axis: tr-bl (↙) can be replaced cleanly by bl-tr (↗).
    ...
    await expect(page.getByRole('heading', { name: 'Axis2 Factory' })).toBeVisible()
...
test.describe('Global exchange sourcing — end-to-end flow', () => {
  test('configure EXCHANGE source, save, and verify exchange panel shows in read-only mode', async ({ page }) => {
    ...
    await expect(page.getByText('Global exchange offers')).toBeVisible()
    await expect(page.locator('.exchange-offer-item')).toHaveCount(9)
  })
  ...
  test('negative path: stale sourcing — all exchange offers blocked in read-only view shows actionable warning', async ({ page }) => {
    ...
    await expect(page.locator('.exchange-offer-item.offer-blocked')).toHaveCount(9)
...
test.describe('Link-aware product picker — STORAGE unit', () => {
  ...
  test('STORAGE unit shows universal buffer info — no product picker', async ({ page }) => {
    // Universal STORAGE: no product picker or resource dropdown should be shown.
    // Instead a friendly info message explains the storage accepts any product.
    const chair = makeChairProduct()
    ...
    // Click on the STORAGE cell at column 1
    await plannedSection.locator('.unit-row').nth(0).locator('.grid-cell').nth(1).click()
  ...
  test('STORAGE unit with no product shows universal info and no picker', async ({ page }) => {
    // STORAGE is now universal: no product selection is required.
    // This test verifies the UI is clean (no picker, just info) when STORAGE has null product.
    const chair = makeChairProduct()
    const player = makeFactoryWithMfgAndStorage({ mfgProductId: 'prod-chair', storageProductId: null })
    const state = setupMockApi(page, { players: [player], products: [chair] })
    state.currentUserId = player.id
    state.currentToken = `token-${player.id}`
    await page.addInitScript((token) => {
    ...
    // Universal info message must be visible — no product selection required
    await expect(page.locator('.config-help', { hasText: /universal buffer/i })).toBeVisible()
...
test.describe('Product picker — contextual ranking section headers', () => {
  ...
  test('full PURCHASE to MFG to STORAGE to PUBLIC_SALES chain: STORAGE is universal, PUBLIC_SALES picker shows connected products', async ({ page }) => {
    ...
    // Click on the STORAGE cell (col 2) — should show universal info, not a picker
    await plannedSection.locator('.unit-row').nth(0).locator('.grid-cell').nth(2).click()
    await expect(page.locator('.config-help', { hasText: /universal buffer/i })).toBeVisible()
    await expect(page.locator('.config-field .picker-trigger')).toHaveCount(0)

    // Click on the PUBLIC_SALES cell (col 3) — picker should still show MFG product
    // (STORAGE acts as a transparent pass-through in the BFS traversal)
    await plannedSection.locator('.unit-row').nth(0).locator('.grid-cell').nth(3).click()
    const publicSalesField = page
      .locator('.config-field')
      .filter({ has: page.getByText('Product Type', { exact: true }) })
      .first()
    await publicSalesField.locator('.picker-trigger').click()
    await expect(page.locator('.product-picker-panel')).toBeVisible()
...
test.describe('Purchase selector list visuals and same-city vendor context', () => {
  ...
  test('same-city vendor cards show price and transit details for a selected resource and sort cheapest first', async ({ page }) => {
    ...
    await expect(vendorCards.first()).toContainText(/Transit €0\.\d{2} per unit/)
    await expect(vendorCards.first().locator('.purchase-vendor-transit-badge')).toBeVisible()
    await expect(vendorCards.first().locator('.purchase-vendor-transit-badge')).toHaveClass(/is-(good|warning|bad|neutral)/)
    await expect(vendorCards.nth(1)).toContainText('Alpha Supply')
    await expect(vendorCards.nth(1)).toContainText('Price €45 per unit')
    await expect(vendorCards.nth(1)).toContainText(/Transit €0\.\d{2} per unit/)
    await expect(vendorCards.nth(1).locator('.purchase-vendor-transit-badge')).toBeVisible()
    await expect(vendorCards.nth(1).locator('.purchase-vendor-transit-badge')).toHaveClass(/is-(good|warning|bad|neutral)/)
  })
})
...
test.describe('Complete building configuration end-to-end journey', () => {
  test('player enters edit mode, sees STORAGE universal info, and cancels without error', async ({ page }) => {
    ...
    // ── Step 3: Click the STORAGE unit and verify universal info message
    const plannedSection = page
      .locator('.grid-section')
      .filter({ has: page.getByRole('heading', { name: 'Planned Upgrade' }) })
      .first()
    ...
    // Universal buffer info must be visible; no picker trigger should exist
    await expect(page.locator('.config-help', { hasText: /universal buffer/i })).toBeVisible()
    await expect(page.locator('.config-field .picker-trigger')).toHaveCount(0)

    // ── Step 4: Cancel editing — building should be in view-only mode
    await page.getByRole('button', { name: /Cancel Editing/i }).click()
    await expect(page.getByRole('button', { name: /Edit Building/i })).toBeVisible()

    // Verify no errors occurred and the building name is still visible
    await expect(page.getByRole('heading', { name: 'Journey Factory' })).toBeVisible()
  })
})
...
test.describe('Global exchange market — per-industry resource coverage', () => {
  ...
  test('Grain (Food Processing input) purchase unit shows exchange offers for all cities', async ({ page }) => {
    ...
    const pragueOffer = page.locator('.exchange-offer-item').filter({ hasText: 'Prague' })
    const pragueTransitText = await pragueOffer.getByText(/Transit:/).textContent()
    expect(pragueTransitText).toMatch(/Transit: [€$£¥₩][\d.,]+/)
...
test.describe('Destination-aware purchase sourcing', () => {
  ...
  test('shows logistics-trap warning when distant city has cheaper sticker but worse delivered price', async ({ page }) => {
    ...
    // Cities 3-8 (NY/London/Beijing/Delhi/Berlin/Warsaw) — clear Wood so abundance defaults to 0.05 (very scarce,
    ...
    state.cities[7]!.resources = []
    state.cities[8]!.resources = []
