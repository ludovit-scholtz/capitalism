...
test.describe('Building edit route tabs', () => {
  test('syncs building edit tabs into URL and restores the selected building tab on refresh', async ({ page }) => {
    ...
    const plannedSection = page
      ...
      .first()
    const unitConfigPanel = page
      .locator('.unit-config')
      .filter({ has: page.getByRole('heading', { name: 'Unit Configuration' }) })
      .first()
    ...
    await expect(unitConfigPanel.locator('[aria-label="Energy Settings"]')).toHaveCount(0)

    await page.getByRole('button', { name: 'Energy Settings', exact: true }).click()
    await expect(page).toHaveURL(/\/building\/building-edit-route\/edit\/layouts(?:\?.*editUnitTab=energy.*)?$/)
    await expect(unitConfigPanel.locator('[aria-label="Energy Settings"]')).toBeVisible()

    await page.getByRole('button', { name: 'Configuration' }).click()
    await expect(unitConfigPanel.locator('[aria-label="Energy Settings"]')).toHaveCount(0)

    await plannedSection.locator('.unit-row').nth(1).locator('.grid-cell').nth(0).click()
    await expect(page.getByRole('button', { name: 'Configuration' })).toHaveClass(/unit-tab-btn--active/)
    await expect(unitConfigPanel.locator('[aria-label="Energy Settings"]')).toHaveCount(0)
  ...
  test('unit energy settings stay in a dedicated tab for multiple unit types', async ({ page }) => {
    const player = makePlayer()
    player.companies.push({
      id: 'company-edit-energy-tab',
      playerId: player.id,
      name: 'Layout Energy Ltd',
      cash: 500000,
      foundedAtUtc: '2026-01-01T00:00:00Z',
      buildings: [
        {
          id: 'building-edit-energy-tab',
          companyId: 'company-edit-energy-tab',
          cityId: 'city-ba',
          type: 'FACTORY',
          name: 'Layout Energy Factory',
          latitude: 48.15,
          longitude: 17.11,
          level: 1,
          powerConsumption: 2,
          ...
          units: [
            {
              id: 'unit-energy-purchase',
              buildingId: 'building-edit-energy-tab',
              unitType: 'PURCHASE',
              gridX: 0,
              gridY: 0,
              level: 1,
    ...
    await page.goto('/building/building-edit-energy-tab')
    await page.getByRole('button', { name: 'Edit Building' }).click()
    await page.getByRole('tab', { name: 'Building Layouts', exact: true }).click()
    ...
    await page.getByRole('button', { name: 'Energy Settings', exact: true }).click()
    await expect(page).toHaveURL(/\/building\/building-edit-energy-tab\/edit\/layouts(?:\?.*editUnitTab=energy.*)?$/)
    await expect(unitConfigPanel.locator('[aria-label="Energy Settings"]')).toBeVisible()
