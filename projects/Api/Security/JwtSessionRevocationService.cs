...
public interface IJwtSessionRevocationService
{
    ...
    Task RevokeAllForPlayerAsync(Guid playerId, string reason, CancellationToken cancellationToken);
    Task RevokeOtherSessionsForPlayerAsync(Guid playerId, string currentJti, string reason, CancellationToken cancellationToken);
    Task<List<PlayerSessionSummary>> GetActiveSessionsAsync(Guid playerId, string? currentJti, CancellationToken cancellationToken);
    Task CleanupExpiredAsync(CancellationToken cancellationToken);
}

public sealed class JwtSessionRevocationService(AppDbContext db) : IJwtSessionRevocationService
{
    ...
    {
        if (!TryReadUserId(principal, out var playerId))
        {
            return true;
        }
        ...
        var tokenRevoked = await db.RevokedTokens
            .AsNoTracking()
        ...
        var session = await db.PlayerSessions.FirstOrDefaultAsync(candidate => candidate.Jti == jti, cancellationToken);
    ...
    public async Task RevokeCurrentAsync(ClaimsPrincipal principal, JwtSecurityToken token, CancellationToken cancellationToken)
    {
        ...
        await RevokeTokenAsync(playerId, ReadJti(token), token.ValidTo, DefaultRevocationReason, cancellationToken);
    }

    public async Task RevokeAllForPlayerAsync(Guid playerId, string reason, CancellationToken cancellationToken)
    {
        var now = DateTime.UtcNow;
        var player = await db.Players.FirstOrDefaultAsync(candidate => candidate.Id == playerId, cancellationToken);
        if (player is null)
        {
            return;
        }
        ...
        var sessions = await db.PlayerSessions
            .Where(session => session.PlayerId == playerId && session.ExpiresAtUtc > now)
        ...
        foreach (var session in sessions)
        {
            ...
            if (!await db.RevokedTokens.AnyAsync(revoked => revoked.Jti == session.Jti, cancellationToken))
            {
    ...
    public async Task RevokeOtherSessionsForPlayerAsync(Guid playerId, string currentJti, string reason, CancellationToken cancellationToken)
    {
        var now = DateTime.UtcNow;
        var sessions = await db.PlayerSessions
            .Where(session => session.PlayerId == playerId && session.ExpiresAtUtc > now && session.Jti != currentJti)
            .ToListAsync(cancellationToken);
    ...
    public async Task<List<PlayerSessionSummary>> GetActiveSessionsAsync(Guid playerId, string? currentJti, CancellationToken cancellationToken)
    {
        var now = DateTime.UtcNow;
        var sessions = await db.PlayerSessions
            .AsNoTracking()
        ...
        return sessions.Select(session => new PlayerSessionSummary
        {
            Jti = session.Jti,
    ...
    public async Task CleanupExpiredAsync(CancellationToken cancellationToken)
    {
        ...
        if (revoked.Count > 0)
        {
            db.RevokedTokens.RemoveRange(revoked);
        }

        var expiredSessions = await db.PlayerSessions
            .Where(session => session.ExpiresAtUtc <= now)
            .ToListAsync(cancellationToken);
        if (expiredSessions.Count > 0)
        {
            db.PlayerSessions.RemoveRange(expiredSessions);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var now = DateTime.UtcNow;
        if (!await db.RevokedTokens.AnyAsync(revoked => revoked.Jti == jti, cancellationToken))
        {
            db.RevokedTokens.Add(new RevokedToken
            {
                Jti = jti,
    ...
    private static bool TryReadUserId(ClaimsPrincipal principal, out Guid playerId)
    {
...
public sealed class PlayerSessionSummary
{
