...
test.describe('Ranking pages', () => {
  ...
  test('admin observability panel shows healthy status when last run succeeded recently', async ({
    page,
  }) => {
    const admin = makePlayer({
      id: 'rank-admin-health',
      email: 'root@example.com',
      displayName: 'Root Admin',
    })

    const recentRunUtc = new Date(Date.now() - 20 * 60 * 1000).toISOString() // 20 minutes ago

    const state = setupMockApi(page, {
      isGlobalAdmin: true,
      rankingModerationEvents: [],
      rankingBounties: [],
      rankingRuns: [
        {
          id: 'run-healthy-1',
          runType: 'EVALUATION',
          status: 'SUCCEEDED',
          startedAtUtc: new Date(Date.now() - 25 * 60 * 1000).toISOString(),
          finishedAtUtc: recentRunUtc,
          processedEvents: 42,
    ...
    await page.goto('/ranking/admin')
    await expect(page.getByRole('heading', { name: 'Evaluator Health' })).toBeVisible()
    await expect(page.locator('.badge-healthy')).toBeVisible()
    await expect(page.locator('.badge-healthy')).toContainText('Healthy')
    await expect(page.locator('.health-age')).toContainText('minutes ago')
  })

  test('admin observability panel shows critical status when last run failed', async ({ page }) => {
    const admin = makePlayer({
      id: 'rank-admin-critical',
      email: 'root@example.com',
      displayName: 'Root Admin',
    })
      ...
      rankingRuns: [
        {
          ...
          finishedAtUtc: new Date(Date.now() - 5 * 60 * 1000).toISOString(),
          processedEvents: 0,
    ...
    await expect(page.locator('.badge-critical')).toContainText('Critical')
    await expect(page.getByRole('alert').first()).toContainText('FAILED')
  })

  test('admin observability panel shows warning when last run was over 2 hours ago', async ({
    page,
  }) => {
    const admin = makePlayer({
      id: 'rank-admin-warning',
      email: 'root@example.com',
      displayName: 'Root Admin',
    })

    const staleRunUtc = new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString() // 3 hours ago
      ...
      rankingRuns: [
        {
          ...
          runType: 'EVALUATION',
          status: 'SUCCEEDED',
          startedAtUtc: new Date(Date.now() - 3.1 * 60 * 60 * 1000).toISOString(),
          finishedAtUtc: staleRunUtc,
          processedEvents: 10,
          rewardRecordsCreated: 2,
    ...
    await expect(page.locator('.badge-warning')).toContainText('Warning')
    await expect(page.getByRole('alert').first()).toContainText('minutes')
  })
