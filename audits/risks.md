...
- **.NET runtime patch currency (CVE-2026-45591 class)**: Mitigated (2026-07-07). Docker images build from the floating `mcr.microsoft.com/dotnet/aspnet:10.0` tag; `api-ci-cd.yml`, `frontend-ci-cd.yml`, and `deploy-stage-k8s.yml` (which builds/deploys MasterApi and master-frontend) now carry a weekly `schedule:` cron trigger in addition to the push trigger, so runtime and base-image security patches ship on a regular cadence even without a code push. Production master deploy remains manual (`workflow_dispatch`) by existing design — an accepted operational choice for controlled production releases, not changed by this fix.
...
Browser-security HTTP headers are configured for both shipped frontend deployments.

- **Game frontend security headers**: Mitigated. `projects/frontend/nginx.conf` sets HSTS, CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, and (as of 2026-07-07, closing a gap where the register previously claimed this incorrectly) `Permissions-Policy`. The CSP `script-src` hash still legitimately covers the inline theme-flash-prevention script in `index.html` (confirmed by recomputing the hash against the built `dist/index.html`) — it does not belong to the gtag bootstrap, which moved to same-origin `/gtag-init.js` under `'self'` in commit `1f7018ad` and needs no hash. `connect-src 'self' https:` / `img-src ... https:` remain deliberately broad for GA, Sentry, and OpenStreetMap; tightening to an explicit host list is a hardening option, not a current gap.
- **Master frontend security headers**: Mitigated. `projects/master-frontend/nginx.conf` sets HSTS, CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, and Permissions-Policy.
- **nginx `add_header` inheritance on non-document locations**: Mitigated (2026-07-07). In both nginx configs, every location that declares its own `add_header` (static-asset regex with `Cache-Control`, game frontend's `/sw.js`, `/health`) now repeats the full security-header set, so asset responses keep `X-Content-Type-Options: nosniff` and HSTS instead of silently losing all inherited headers. Verified with `npm run test:security-headers` in both frontends.
...
- **OIDC metadata HTTPS requirement**: Mitigated (2026-07-07). Both base `appsettings.json` files now set `"RequireHttpsMetadata": true`, `MasterApi/Security/BiatecOidcOptions.RequireHttpsMetadata` now defaults to `true`, and a fail-fast non-Development/Testing startup guard (`RequiredSecretsStartupGuard.TryGetUnsafeOidcHttpsMetadataReason`, wired into both `Program.cs`) blocks startup when `BiatecOidc:Enabled=true` and either `RequireHttpsMetadata=false` or the Authority is not an HTTPS URL. Development/Testing still only warn, preserving local override flexibility. Regression tests cover both throw and no-throw paths in `Api.Tests` and `MasterApi.Tests`.
