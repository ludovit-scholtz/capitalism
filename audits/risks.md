...
- **Game frontend dependencies**: Partially mitigated. As of 2026-06-20, `npm audit --omit=dev` reports 1 moderate vulnerability: `dompurify ≤3.4.10` (8 CVEs; IN_PLACE mode, setConfig/hook pollution, Trusted Types). The app does not use the vulnerable DOMPurify APIs (standard `sanitize()` only), so direct exploitability is low, but an upgrade to ≥3.4.11 via `npm audit fix` is required. Full `npm audit` additionally reports dev-only findings in Vite (NTLMv2 disclosure and fs.deny bypass on Windows dev server) and build toolchain deps.
- **Master frontend dependencies**: Partially mitigated. As of 2026-06-20, same DOMPurify ≤3.4.10 production advisory applies. Same fix: `npm audit fix` in `projects/master-frontend`. Dev-only findings mirror the game frontend.
