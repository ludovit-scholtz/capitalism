...
- Every user can become a referal if he fills in the real name and his tax domicil.
- In the master frontend is the referal dashboard where user can activate the referal account.
- One user can create multiple referal codes. First referal code is autogenerated - 8 alphanumerical string.
- Any user can fill in the referal code of the other player. This way he will receive a referal discount on subscription and referal will receive share of the subscription and share of the second level subscription.
- When purchasing a subscription and user does not have active referal, user is prompted to add the referal code. There is no information that the referal code will give him discount, but person who recommended the game should promote it.
- In the referal dashboard is shown the number of registered users under the specific referal code, number of second level referals registrations, number of active subscriptions, and number of second level active referal subscriptions
